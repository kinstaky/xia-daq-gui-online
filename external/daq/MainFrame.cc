...
#include "external/pixie/sys/pixie16sys_export.h"
...
void MainFrame::ExpModeInit()
{
  ...
  expmodeinitwrite.open("parset/exp/DSPpar.thre");//ios::bin ios::app
  if(!expmodeinitwrite.is_open())
    {
      std::cout<<"can't open file -- parset/exp/DSPpar.thre"<<std::endl;
      return;
    }
